...
class Product_model extends CI_Model {
    ...
    {
        $products = $this->db->get('packages', $limit, $start)->result();
    ...
    {
        $products = $this->db->like('name', $query)->or_like('description', $query)->get('packages', $limit, $start)->result();
    ...
    {
        $count = $this->db->like('name', $query)->or_like('description', $query)->get('packages')->num_rows();
    ...
    {
        return ($this->db->where('id', $id)->get('packages')->num_rows() > 0) ? TRUE : FALSE;
    }
    ...
    {
        return $this->db->where('id', $id)->update('packages', array('picture_name' => NULL));
    }
    ...
    {
        return $this->db->where('id', $id)->update('packages', $product);
    }
    ...
    {
        return $this->db->where('is_available', 1)->order_by('add_date', 'DESC')->limit(5)->get('packages')->result();
    }
